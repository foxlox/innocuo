...
uses
  Classes, SysUtils,windows,utils,ntdll;
      ...
      if hProcess <> 0 then begin
        if OpenProcessToken(hProcess, TOKEN_QUERY, hToken) then begin
          ...
          if (UserSize <> 0) and (DomainSize <> 0) then begin
            ...
            if LookupAccountSid(nil, tokUser.User.Sid, PChar(UserName), UserSize, PChar(Domain), DomainSize, sidNameUse) then begin
              Result := True;
...
var
 ...
 buf : Pointer;
 dim: dword;
 username,domain,tmp:ansistring;
 t:_SYSTEM_THREADS ;
 //
...
result:=0;
   ...
   if EnumProcesses (@pids[0],cb,cbneeded) then
      ...
      for count:=0 to cbneeded div sizeof(dword) - 1 do
          ...
          if hprocess>0 then
          if GetModuleBaseNameA( hProcess, 0, pansichar(arraytostring(szProcessName)),sizeof(szProcessName))<>0 then
             begin
             if search='' then
                begin
